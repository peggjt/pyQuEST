...
class Gradients:
    ...
    def gradient(
        ...
        for n, adj_gate in enumerate(circ.inverse):
            ...
            overlap = 2 * lambda_state * mu_state
            if n > 0:
                lambda_state.apply_operator(adj_gate)
            gradient_register = gradient_register + (overlap * mu_state)
    ...
    def nat_gradient(
        ...
        n_qbits = self.infer_num_qubits(circ)
        psi = Register(n_qbits)
        psi.apply_circuit(circ)

        deriv_states = []
        phi_state = Register(copy_reg=psi)
        for adj_gate in circ.inverse:
            orig_gate = adj_gate.inverse
            phi_state.apply_operator(adj_gate)
            mu = Register(copy_reg=phi_state)
            mu = self.unitary_differentiation(mu, orig_gate)
            deriv_states.append(mu)
        deriv_states = deriv_states[::-1]

        natural_gradient_register = Register(n_qbits)
        natural_gradient_register.init_blank_state()
        for coeff, mu in zip(coeffs, deriv_states):
            natural_gradient_register = natural_gradient_register + (coeff * mu)
